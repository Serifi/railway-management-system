<template>
  <div class="grid grid-cols-2 gap-4">
    <div class="flex flex-col col-span-2 lg:col-span-1">
      <label for="trackGauge" class="mb-1">Spurweite</label>
    ...
    <div class="flex flex-col col-span-2 lg:col-span-1">
      <label for="type" class="mb-1">Typ</label>
    ...
    <div v-if="carriage.type === 'Railcar'" class="flex flex-col col-span-2">
      <label for="maxTractiveForce" class="mb-1">Maximale Zugkraft in kN</label>
      <InputNumber id="maxTractiveForce" v-model="carriage.maxTractiveForce" placeholder="Wert eingeben..." :disabled="isDisabled('maxTractiveForce')"/>
    </div>

    <div v-if="carriage.type === 'PassengerCar'" class="flex flex-col col-span-2 lg:col-span-1">
      <label for="numberOfSeats" class="mb-1">Sitzplätze</label>
      <InputNumber id="numberOfSeats" v-model="carriage.numberOfSeats" placeholder="Wert eingeben..." :disabled="isDisabled('numberOfSeats')"/>
    </div>

    <div v-if="carriage.type === 'PassengerCar'" class="flex flex-col col-span-2 lg:col-span-1">
      <label for="maxWeight" class="mb-1">Maximales Gewicht in kg</label>
      <InputNumber id="maxWeight" v-model="carriage.maxWeight" placeholder="Wert eingeben..." :disabled="isDisabled('maxWeight')"/>
    </div>
  </div>
</template>
