<template>
  <List :items="trainStations" :getKey="getTrainStationKey" :rowsPerPage="5"
        @create="toggleCreateDialog" @edit="toggleEditDialog" @delete="deleteTrainStation">
    <template #title="{ item }">
      {{ item.stationName }}
    </template>
  ...
  <ScotDialog :visible="createDialogVisible" type="create" header="Station erstellen" :disable-action="disableAction"
              @update:visible="createDialogVisible = $event" @action="createTrainStation" @cancel="toggleCreateDialog">
    <ScotTrainStation @update:trainStation="updateTrainStation"/>
  </ScotDialog>

  <ScotDialog :visible="editDialogVisible" type="edit" header="Station bearbeiten" :disable-action="disableAction"
              @update:visible="editDialogVisible = $event" @action="editTrainStation" @cancel="toggleEditDialog">
